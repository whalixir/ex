...
const TGJU_CODES = ['USD','AED','OMR','SAR','GBP','CHF','BTC','ETH','SOL','GOLD'];
const LOGO_SRC = '';
let filter = 'all';
let chartInstance = null;

const $  = id => document.getElementById(id);
const fN = (n,d=0) => { if(isNaN(n)) return '0'; const v=Math.round(n*Math.pow(10,d))/Math.pow(10,d); return v.toLocaleString('en-US',{minimumFractionDigits:d,maximumFractionDigits:d}); };
...
// ══════════════════════════════════════════════════════════════════
// سیستم صدا — کلیک / تأیید / PIN
// ══════════════════════════════════════════════════════════════════
const _AC = (()=>{
  let ctx=null;
  return ()=>{ if(!ctx) ctx=new(window.AudioContext||window.webkitAudioContext)(); return ctx; };
})();
...
// اتصال صدا به همه دکمه‌ها و آیکون‌ها بعد از بارگذاری
function initSounds(){
  // PIN keys
  document.querySelectorAll('.pb[data-n]').forEach(b=>{
    b.addEventListener('click',()=>playPinKey(),{passive:true});
  });
  // همه دکمه‌های نوار پایین
  document.querySelectorAll('.mob-nav-btn,.nb,.vab').forEach(b=>{
    b.addEventListener('click',()=>playIconTap(),{passive:true});
  });
  // دکمه ثبت معامله
  const sub=$('sub'); if(sub) sub.addEventListener('click',()=>{},{ passive:true });
}
...
(function injectGlobalStyles(){
  ...
  body.light{
    ...
    --br:rgba(79,140,255,.18);
    --tx1:#0d1b35;--tx2:#3a5080;--tx3:#8099bb;
    --sh:0 8px 32px rgba(79,140,255,.12);
    --glass:rgba(255,255,255,.70);
    --glass-border:rgba(79,140,255,.20);
  }

  /* ── منوی پایین شناور/شیشه‌ای ──────────────────── */
  .mob-nav{
  ...
  .mob-nav-btn{
    ...
    cursor:pointer!important; font-size:.64rem!important; transition:all .2s ease!important;
  ...
  /* ── داشبورد کارت‌های سود/زیان ─────────── */
  .pv-card{
    background:linear-gradient(135deg,rgba(79,140,255,.15),rgba(111,163,255,.05))!important;
    border:1px solid rgba(79,140,255,.3)!important; border-radius:20px!important;
    box-shadow:0 4px 24px rgba(79,140,255,.15)!important;
  }
...
async function fetchTgjuRates(showStatus=true){
  const statusEl=$('rateFetchStatus');
  const tsEl=$('rateTs');
  const btn=$('rfb');
  ...
  try{
    const r=await fetch('https://ex.4dgwb9f5dh.workers.dev/api/tgju?_='+Date.now(),{
      cache:'no-store',
    ...
    if(updated>0){
      ...
      changed.forEach(code=>{
        ...
        if(card){
          card.classList.add('rate-flash');
          setTimeout(()=>card.classList.remove('rate-flash'),1200);
        }
      });
      ...
      const errMsg=errCount>0?' | '+errCount+' مورد دریافت نشد':'';
      if(showStatus){
      ...
      const errs=(data._errors||[]).join(' | ');
      throw new Error('هیچ نرخی دریافت نشد. '+errs);
    }
  }catch(err){
    if(showStatus){
    ...
    toast('خطا در دریافت نرخ‌ها','err');
  }finally{
...
function calcAll(){
  ...
  for(const code of Object.keys(CUR))
    data[code]={buyAmt:0,buyToman:0,sellAmt:0,sellToman:0};
  for(const tx of txs){
  ...
  const totalProfitAED = totalProfitToman/aedRate;
  return{result,totalProfitToman,totalProfitAED,totalInventoryValue};
}

// ── MONTHLY DATA برای نمودار ─────────────────────────────────────
...
function jalaliMonth(yr,mo){
  ...
  let jMo=mo-3; let jYr=yr-621;
  if(jMo<=0){jMo+=12;jYr--;}
  return jalaliMonths[(jMo-1)%12]+' '+jYr;
}
...
function renderDashboard(){
  ...
  for(const [code,d] of Object.entries(result)){
    if(d.inventory>0 && rates[code]>0){
      totalInventoryToman += d.inventory * rates[code];
    }
  }
  ...
  if(balGrid){
    ...
    balGrid.innerHTML=show.map(code=>{
      const d=result[code];
      const inv=d?d.inventory:0;
      const invToman=inv>0?(inv*rates[code]):0;
      const c=CUR[code];
      const dec=code==='BTC'?4:code==='OMR'?3:2;
      const cls=inv>0?'pos':inv<0?'neg':'neu';
      return `<div class="inv-card" style="padding:14px 16px">
        <div class="inv-header" style="margin-bottom:8px">
          <div class="inv-flag" style="font-size:1.6rem">${c.flag}</div>
          <div><div class="inv-name" style="font-size:.88rem">${c.name}</div></div>
        </div>
...
function renderInventory(){
  ...
  const cards=Object.entries(CUR).map(([code,c])=>{
    ...
    return `<div class="inv-card">
      ...
      <div class="inv-row"><span class="inv-label">موجودی</span>
        <span class="inv-val ${invC}">${fN(d.inventory,dec)} ${c.unit}</span></div>
      <div class="inv-row"><span class="inv-label">کل خرید</span>
        <span class="inv-val neu">${fN(d.buyAmt,dec)} ${c.unit}</span></div>
      <div class="inv-row"><span class="inv-label">کل فروش</span>
        <span class="inv-val neu">${fN(d.sellAmt,dec)} ${c.unit}</span></div>
      <div class="inv-row"><span class="inv-label">میانگین خرید</span>
        <span class="inv-val neu">${fN(d.avgBuy)} تومان</span></div>
      <div class="inv-row"><span class="inv-label">سود / زیان (تومان)</span>
        <span class="inv-val ${pC}">${pS}${fN(Math.abs(d.profitToman))} تومان</span></div>
      <div class="inv-row"><span class="inv-label">سود / زیان (درهم)</span>
        <span class="inv-val ${paC}">${paS}${fN(Math.abs(profitAED),2)} درهم</span></div>
    </div>`;
  }).join('');
...
function renderChart(){
  const{labels,profitToman,profitAED}=chartPeriod==='daily'?dailyData():monthlyData();
  const canvas=$('myChart');
  const isDark=document.body.classList.contains('dark');
  const gridColor=isDark?'rgba(255,255,255,.07)':'rgba(0,0,0,.07)';
  const textColor=isDark?'#90aec9':'#2c5282';
      ...
      datasets:[
        {
          ...
          label:'سود تومانی',
          data:profitToman,
          backgroundColor:profitToman.map(v=>v>=0?'rgba(38,215,130,.7)':'rgba(255,82,113,.7)'),
          borderColor:profitToman.map(v=>v>=0?'#26d782':'#ff5271'),
          borderWidth:1,
          borderRadius:4,
          yAxisID:'y',
          order:2
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{
          ...
          callbacks:{
            label:ctx=>{
              if(ctx.dataset.label==='سود تومانی')
                return ' '+fN(ctx.raw)+' تومان';
              return ' '+fN(ctx.raw,2)+' درهم';
            }
          }
        }
      },
      scales:{
        ...
        y:{
          position:'right',
          ticks:{color:textColor,font:{family:'Vazirmatn'},callback:v=>fN(v)+' ت'},
...
const chartDailyBtn=$('chartDaily');
const chartMonthlyBtn=$('chartMonthly');
...
(function initChartLandscape(){
  ...
  function exitFullscreen(){
    ...
    fsHost.style.display = 'none';
    document.body.classList.remove('wx-chart-fs-active');
    resetCanvasSize();
    requestAnimationFrame(()=>requestAnimationFrame(renderChart));
  }
  function applyChartOrientation(){
    ...
    const shouldFs = isLandscape() && isMobileSize() && onChartTab;
    if(shouldFs) enterFullscreen(); else exitFullscreen();
  }
  window.addEventListener('orientationchange', ()=>setTimeout(applyChartOrientation,250));
  window.addEventListener('resize', ()=>{
    clearTimeout(window._wxChartResizeT);
    window._wxChartResizeT = setTimeout(applyChartOrientation,150);
  });
  const origTab2 = window.tab;
  if(typeof origTab2==='function'){
    window.tab = function(id){
      origTab2(id);
      if(id==='chart') setTimeout(applyChartOrientation,120);
      else exitFullscreen();
    };
  }
...
function renderRates(){
  $('rg').innerHTML=Object.entries(CUR).map(([c,v])=>`
    <div class="rc" data-c="${c}">
      <div class="rf">${v.flag}</div>
      <div class="rn">${v.name}</div>
      <div class="rco">${c}</div>
      <div class="rv">${fN(rates[c])}</div>
      <div class="ru">تومان / ${v.unit}</div>
      <div class="reh">کلیک برای ویرایش دستی</div>
    </div>`).join('');
  $('rg').querySelectorAll('.rc').forEach(card=>{
    card.onclick=async()=>{
      const c=card.dataset.c;
      if(card.querySelector('.rei')) return;
...
function txHTML(tx){
  ...
  return `<div class="ti-wrap" data-id="${tx.id}">
    ...
    <div class="ti">
      ...
      <div class="tin">
        <div class="tm"><span class="tt2">${tl} ${c.name}</span>
...
function bindDel(el){
  el.querySelectorAll('.ti-wrap').forEach(wrap=>{
    const inner = wrap.querySelector('.ti');
    const delBg  = wrap.querySelector('.ti-del-bg');
    ...
    if(delBg){
      delBg.addEventListener('click', async e=>{
        ...
        wrap.style.transition='max-height .3s ease, opacity .3s ease';
        wrap.style.opacity='0';
        setTimeout(()=>wrap.style.maxHeight='0', 50);
        setTimeout(async()=>{
          render();
          try{ await api('/transactions/'+id,{method:'DELETE'}); }catch(_){}
          toast('تراکنش به سطل آشغال رفت 🗑','');
        },320);
      });
    }
  });
...
$('ca').onclick=async()=>{
  if(!confirm('همه تراکنش‌ها حذف شوند؟'))return;
...
$('sub').onclick=async()=>{
  ...
  toast('✅ معامله ثبت شد!','ok');
  playSuccess();
  tab('dashboard');
  try{
    const r=await api('/transactions',{method:'POST',body:JSON.stringify(tx)});
    if(r.id){tx.id=r.id;localStorage.setItem('wx_tx',JSON.stringify(txs));}
  }catch(_){}
};
...
function renderTrash(){
  ...
  if(!el) return;
  if(!trash.length){
    el.innerHTML=`<div class="es"><div style="font-size:3rem">🗑</div><div class="etag" style="margin-top:8px">سطل خالی است</div></div>`;
    return;
  }
  el.innerHTML=trash.map(tx=>{
    ...
    return `<div class="ti" style="opacity:.75">
      ...
      <div class="tin">
        <div class="tm">
          <span class="tt2">${tx.type==='buy'?'خرید':'فروش'} ${c.name}</span>
          <span class="ta" style="color:var(--tx3)">${fN(tx.amount)} ${c.unit}</span>
        </div>
        <div class="tsb"><span class="tr">نرخ: ${fN(tx.rate)} تومان</span><span class="td2">${fD(tx.ts)}</span></div>
        ${tx.note?`<div class="tn">📝 ${tx.note}</div>`:''}
      </div>
  ...
  el.querySelectorAll('.trash-restore').forEach(btn=>{
    btn.onclick=async()=>{
      ...
      const tx=trash.find(t=>String(t.id)===String(id));
      if(!tx) return;
      ...
      localStorage.setItem('wx_tx',JSON.stringify(txs));
      try{ await api('/transactions',{method:'POST',body:JSON.stringify(restored)}); }catch(_){}
      render();
      toast('تراکنش بازگردانده شد ✓','ok');
    };
  });
  ...
  el.querySelectorAll('.trash-del').forEach(btn=>{
    btn.onclick=()=>{
      ...
      localStorage.setItem('wx_trash',JSON.stringify(trash));
      render();
      toast('تراکنش برای همیشه حذف شد','');
    };
  });
}
...
if(restoreAllBtn){
  restoreAllBtn.onclick=async()=>{
    ...
    for(const tx of trash){
      const r={...tx}; delete r.deletedAt;
      txs.unshift(r);
      try{ await api('/transactions',{method:'POST',body:JSON.stringify(r)}); }catch(_){}
    }
    trash=[];
    localStorage.setItem('wx_trash','[]');
    localStorage.setItem('wx_tx',JSON.stringify(txs));
    render();
    toast('همه تراکنش‌ها بازگردانده شدند ✓','ok');
  };
}
...
function enter(){
  if(pin===PIN){
    ...
    setTimeout(()=>{
      ...
      if(!window._wxAutoRefresh){
        window._wxAutoRefresh = setInterval(()=>fetchTgjuRates(false), 30000);
      }
      initSounds();
      injectBoursUI();
      fixNavBar();
    ...
    pin=''; dots();
    setTimeout(()=>$('pe').textContent='',2000);
  }
}
document.querySelectorAll('.pb[data-n]').forEach(b=>{
...
let boursChartT = null, boursChartD = null;
...
function renderBoursCharts(recT, recD){
  ...
  if(ctxTpl && recT.length>0){
    if(boursChartT) boursChartT.destroy();
    const labels=recT.map(r=>r.date);
    const plToman=recT.map(r=>r.pl);
    const plAED=recT.map(r=>r.pl/aedRate);
    boursChartT=new Chart(ctxTpl,{
      type:'line',
      data:{
        labels,
        datasets:[
          {
            ...
            data:plToman,
            borderColor:ctx=>plToman.map(v=>v>=0?'#26d782':'#ff5271')[ctx.dataIndex]||'#26d782',
            borderColor:'#26d782',
            backgroundColor:'rgba(38,215,130,.1)',
            borderWidth:2.5,
            pointRadius:5,
            pointBackgroundColor:plToman.map(v=>v>=0?'#26d782':'#ff5271'),
            tension:.3,
            yAxisID:'y',
            segment:{
              borderColor:ctx=>{
                const v=plToman[ctx.p1DataIndex];
                return v>=0?'#26d782':'#ff5271';
              }
            }
          },
          {
            ...
            borderWidth:2,
            pointRadius:4,
            pointBackgroundColor:plAED.map(v=>v>=0?'#4f8cff':'#ff5271'),
            tension:.3,
            yAxisID:'y2',
            segment:{
              borderColor:ctx=>{
                const v=plAED[ctx.p1DataIndex];
                return v>=0?'#4f8cff':'#ff5271';
              }
            }
          }
        ]
  ...
  // TSETMC portfolio bar
  if(ctxTpf && recT.length>0){
    const existingChart=Chart.getChart(ctxTpf);
    if(existingChart) existingChart.destroy();
    new Chart(ctxTpf,{
      type:'bar',
      data:{
        labels:recT.map(r=>r.date),
        datasets:[{
          label:'ارزش پرتفوی (تومان)',
          data:recT.map(r=>r.portfolio),
          backgroundColor:'rgba(79,140,255,.7)',
          borderColor:'#4f8cff',
          borderWidth:1,
          borderRadius:6
        }]
  ...
  if(ctxD && recD.length>0){
    const existingD=Chart.getChart(ctxD);
    if(existingD) existingD.destroy();
    const labelsD=recD.map(r=>r.date);
    const plD=recD.map(r=>r.pl);
    const pfD=recD.map(r=>r.portfolio);
    new Chart(ctxD,{
      data:{
        labels:labelsD,
        datasets:[
          ...
          },
          {
            ...
            label:'سود/زیان (درهم)',
            data:plD,
            borderColor:ctx=>plD.map(v=>v>=0?'#26d782':'#ff5271')[ctx.dataIndex]||'#26d782',
            borderColor:'#f5c842',
            backgroundColor:'rgba(245,200,66,.1)',
            borderWidth:2.5,
            pointRadius:5,
            pointBackgroundColor:plD.map(v=>v>=0?'#26d782':'#ff5271'),
            tension:.3,
            fill:false,
            yAxisID:'y2',
            order:1,
...
function renderBours(){
  ...
  <div class="bours-table-wrap">
    <table class="bours-table">
      <thead><tr><th>تاریخ</th><th>ارزش (تومان)</th><th>معادل درهم</th><th>واریز</th><th>برداشت</th><th>سود/زیان</th><th>یادداشت</th><th></th></tr></thead>
      <tbody>${[...recT].reverse().map(r=>`
        <tr>
          <td>${r.date}</td>
          <td class="num">${fN(r.portfolio)}</td>
          <td class="num" style="color:var(--tx3);font-size:.8rem">${fN(tomanToAED(r.portfolio),2)} د</td>
          ...
          <td style="font-size:.78rem">${r.note||''}</td>
  ...
  <div class="bours-table-wrap">
    <table class="bours-table">
      <thead><tr><th>تاریخ</th><th>ارزش (درهم)</th><th>واریز</th><th>برداشت</th><th>سود/زیان</th><th>یادداشت</th><th></th></tr></thead>
      <tbody>${[...recD].reverse().map(r=>`
        <tr>
          <td>${r.date}</td>
  ...
  <div class="bours-page">
    ...
    <div class="bours-summary">
      <div class="bours-sum-card">
        <div class="bours-sum-label">سود/زیان TSETMC</div>
    ...
    <div class="bours-section bours-chart-section">
      <div class="bours-sec-header"><div class="bours-sec-title">📊 نمودار TSETMC — سود/زیان</div></div>
      <div style="height:220px;padding:12px"><canvas id="boursChartTPL"></canvas></div>
      <div class="bours-sec-header" style="margin-top:4px"><div class="bours-sec-title">📊 ارزش پرتفوی TSETMC</div></div>
      <div style="height:200px;padding:12px"><canvas id="boursChartTPF"></canvas></div>
    </div>`:''}
    ...
    <div class="bours-section bours-chart-section">
      ...
      <div style="height:240px;padding:12px"><canvas id="boursChartD"></canvas></div>
    </div>`:''}
...
function injectBoursUI(){
  ...
  .bours-title{ font-size:1.35rem; font-weight:700; color:var(--ac); }
  .bours-subtitle{ font-size:.78rem; color:var(--tx3); margin-top:3px; }

  .bours-summary{ display:grid; grid-template-columns:repeat(auto-fill,minmax(155px,1fr)); gap:10px; margin:12px 0 16px; }
  .bours-sum-card{
    background:linear-gradient(135deg,rgba(79,140,255,.12),rgba(79,140,255,.04));
    border:1px solid rgba(79,140,255,.2); border-radius:16px;
    padding:12px 14px; text-align:center;
    backdrop-filter:blur(12px);
  }
  .bours-sum-label{ font-size:.72rem; color:var(--tx3); margin-bottom:4px; }
  .bours-sum-val{ font-size:1rem; font-weight:700; }
  .bours-sum-sub{ font-size:.7rem; color:var(--tx3); margin-top:2px; }

  .bours-section{
    background:linear-gradient(135deg,var(--glass),rgba(255,255,255,.01));
    backdrop-filter:blur(16px); border:1px solid var(--glass-border);
    border-radius:18px; margin-bottom:14px; overflow:hidden;
  }
  .bours-chart-section{ background:linear-gradient(135deg,rgba(79,140,255,.06),rgba(79,140,255,.01)); }
  .bours-sec-header{ display:flex; align-items:center; justify-content:space-between; padding:13px 16px; border-bottom:1px solid var(--br); }
  .bours-sec-title{ font-size:.88rem; font-weight:600; color:var(--tx1); }

  .bours-add-btn{ background:linear-gradient(135deg,var(--ac),#3a6fd8); color:#fff; border:none; border-radius:10px; padding:7px 15px; font-family:var(--font); font-size:.8rem; cursor:pointer; box-shadow:0 2px 10px rgba(79,140,255,.3); }
  .bours-add-btn:active{ transform:scale(.94); }

  .bours-form{ padding:14px 16px; border-bottom:1px solid var(--br); }
  .bours-form-row{ display:flex; align-items:center; gap:8px; margin-bottom:8px; flex-wrap:wrap; }
  .bours-form-row label{ font-size:.78rem; color:var(--tx3); min-width:160px; text-align:right; }
  .bours-form-row input{ flex:1; min-width:130px; background:rgba(255,255,255,.05); border:1px solid var(--br); border-radius:10px; color:var(--tx1); font-family:var(--font); font-size:.86rem; padding:7px 10px; outline:none; }
  ...
  .bours-btn-save{ background:linear-gradient(135deg,var(--ac),#3a6fd8); color:#fff; border:none; border-radius:10px; padding:9px 22px; font-family:var(--font); font-size:.84rem; cursor:pointer; }
  .bours-btn-cancel{ background:rgba(255,255,255,.06); color:var(--tx2); border:1px solid var(--br); border-radius:10px; padding:9px 18px; font-family:var(--font); font-size:.84rem; cursor:pointer; }

  .bours-table-wrap{ overflow-x:auto; padding:4px; }
  .bours-table{ width:100%; border-collapse:collapse; font-size:.8rem; }
  ...
  .bours-table td{ padding:9px 10px; border-bottom:1px solid rgba(255,255,255,.04); white-space:nowrap; color:var(--tx1); }
  .bours-table tr:last-child td{ border-bottom:none; }
  .bours-table td.num{ text-align:left; direction:ltr; }
  .bours-del-btn{ background:none; border:none; cursor:pointer; font-size:.95rem; opacity:.5; }
  .bours-del-btn:hover{ opacity:1; }
  .bours-empty{ padding:24px; text-align:center; color:var(--tx3); font-size:.85rem; }
  `;
  document.head.appendChild(s);

  // اضافه کردن تب بورس در منوی پایین
  const mobNav=document.querySelector('.mob-nav');
  if(mobNav && !document.querySelector('.mob-nav-btn[data-tab="bours"]')){
    // پیدا کردن دکمه "معامله"
    const tradeBtns=[...mobNav.querySelectorAll('.mob-nav-btn')];
    const tradeBtn=tradeBtns.find(b=>b.dataset.tab==='trade'||b.textContent.includes('معامله'));
    ...
    if(tradeBtn && tradeBtn.nextSibling){
      mobNav.insertBefore(btn, tradeBtn.nextSibling);
    } else {
...
function injectPDFButton(){
  ...
  // پیدا کردن داشبورد و اضافه کردن دکمه
  const dash=document.getElementById('tab-dashboard');
  if(!dash) return;

  // دکمه PDF را در بالای داشبورد اضافه می‌کنیم
  const pdfBtn=document.createElement('button');
  pdfBtn.id='wxPdfBtn';
  pdfBtn.innerHTML='📄 دانلود گزارش PDF';
  pdfBtn.style.cssText=`
    display:block; margin:10px auto 6px; padding:10px 22px;
    background:linear-gradient(135deg,#4f8cff,#3a6fd8);
    color:#fff; border:none; border-radius:12px;
    font-family:var(--font); font-size:.85rem; cursor:pointer;
    box-shadow:0 4px 16px rgba(79,140,255,.35); width:calc(100% - 24px);
    max-width:400px;
  `;
  pdfBtn.onclick=()=>{ playClick(); generatePDF(); };
  dash.prepend(pdfBtn);
}
...
async function generatePDF(){
  toast('در حال تولید گزارش...','');

  // بارگذاری jsPDF اگر موجود نیست
  if(!window.jspdf){
  ...
  const W=210, H=297;
  const now=new Date();
  const {result,totalProfitToman,totalProfitAED,totalInventoryValue}=calcAll();
  const aedRate=rates['AED']||1;
  ...
  doc.text('WHALIXIR',W/2,23,{align:'center'});
  doc.setFontSize(8);
  doc.setTextColor(144,174,201);
  doc.text('BY SHAMSADDIN MOLLAEI',W/2,29,{align:'center'});
  doc.setFontSize(9);
  doc.setTextColor(79,140,255);
  doc.text('گزارش مالی جامع',W/2,35,{align:'center'});

  // تاریخ
  doc.setFontSize(8);
  doc.setTextColor(144,174,201);
  doc.text('تاریخ: '+toJalali(now)+'  |  ساعت: '+now.toLocaleTimeString('fa-IR'),W/2,42,{align:'center'});

  let y=50;

  // ── تابع ترسیم بخش ──
  function section(title, color=[20,35,65]){
    doc.setFillColor(...color);
    doc.roundedRect(10,y,W-20,8,2,2,'F');
    doc.setTextColor(79,140,255);
    doc.setFontSize(10);
    doc.setFont('helvetica','bold');
    doc.text(title,W-15,y+5.5,{align:'right'});
    y+=11;
  }

  function row(label,value,valueColor=[226,240,255]){
    doc.setFillColor(17,28,46);
    doc.rect(10,y,W-20,7,'F');
    doc.setTextColor(144,174,201);
    doc.setFontSize(8);
    doc.setFont('helvetica','normal');
    doc.text(label,W-14,y+4.8,{align:'right'});
    doc.setTextColor(...valueColor);
    doc.text(String(value),24,y+4.8,{align:'left'});
    y+=7.5;
  }

  // ── ۱. خلاصه مالی ──
  section('خلاصه وضعیت مالی');
  const plColor=totalProfitToman>=0?[38,215,130]:[255,82,113];
  row('سود/زیان کل (تومان)',(totalProfitToman>=0?'+':'')+fN(Math.abs(totalProfitToman))+' تومان', plColor);
  row('سود/زیان کل (درهم)',(totalProfitAED>=0?'+':'')+fN(Math.abs(totalProfitAED),2)+' درهم', plColor);
  row('ارزش کل دارایی‌ها',fN(Math.round(totalInventoryValue))+' تومان');
  const totalBuy=Object.values(result).reduce((s,d)=>s+d.buyToman,0);
  const ret=totalBuy>0?((totalProfitToman/totalBuy)*100).toFixed(1):0;
  row('بازدهی کل',ret+'٪',(parseFloat(ret)>=0?[38,215,130]:[255,82,113]));
  y+=4;
  ...
  if(recT.length){
    section('بورس تهران — TSETMC');
    const tPlColor=totalPLT>=0?[38,215,130]:[255,82,113];
    row('سود/زیان کل TSETMC',(totalPLT>=0?'+':'')+fN(Math.abs(totalPLT))+' تومان', tPlColor);
    row('معادل درهم',fN(tomanToAED(totalPLT),2)+' درهم', tPlColor);
    if(recT[recT.length-1]) row('ارزش جاری پرتفوی',fN(recT[recT.length-1].portfolio)+' تومان');
    y+=4;
  }
  ...
  if(recD.length){
    section('بورس دبی — DFM');
    const dPlColor=totalPLD>=0?[38,215,130]:[255,82,113];
    row('سود/زیان کل DFM',(totalPLD>=0?'+':'')+fN(Math.abs(totalPLD),2)+' درهم', dPlColor);
    if(recD[recD.length-1]) row('ارزش جاری پرتفوی',fN(recD[recD.length-1].portfolio,2)+' درهم');
    y+=4;
  }

  // ── ۴. ارزها ──
  const currRows=Object.entries(CUR).map(([code,c])=>{
    const d=result[code];
    if(!d||d.buyAmt===0) return null;
    return{code,c,d};
  }).filter(Boolean);

  if(currRows.length){
    section('پرتفوی ارزها');
    currRows.forEach(({code,c,d})=>{
      const dec=code==='BTC'?6:code==='GOLD'?3:2;
      const profitAED=d.profitToman/aedRate;
      const color=d.profitToman>=0?[38,215,130]:[255,82,113];
      row(c.name+' ('+code+')',
        'موجودی: '+fN(d.inventory,dec)+' | سود: '+(d.profitToman>=0?'+':'')+fN(Math.abs(d.profitToman))+' ت',
        color);
      if(y>265){ doc.addPage(); doc.setFillColor(11,17,32); doc.rect(0,0,W,H,'F'); y=15; }
    });
    y+=4;
  }

  // ── ۵. خلاصه نهایی ──
  if(y>255){ doc.addPage(); doc.setFillColor(11,17,32); doc.rect(0,0,W,H,'F'); y=15; }
  section('خلاصه نهایی',[15,40,80]);
  const totalPortfolio=totalInventoryValue+(recD.length&&recD[recD.length-1]?recD[recD.length-1].portfolio*aedRate:0);
  row('سود کل ارزها',(totalProfitToman>=0?'+':'')+fN(Math.abs(totalProfitToman))+' تومان');
  if(recT.length) row('سود کل TSETMC',(totalPLT>=0?'+':'')+fN(Math.abs(totalPLT))+' تومان');
  if(recD.length) row('سود کل DFM',(totalPLD>=0?'+':'')+fN(Math.abs(totalPLD),2)+' درهم');
  row('ارزش کل دارایی‌ها',fN(Math.round(totalPortfolio))+' تومان');
  row('بازده کل',ret+'٪',(parseFloat(ret)>=0?[38,215,130]:[255,82,113]));

  // ── فوتر ──
  doc.setFillColor(20,35,65);
  doc.rect(0,H-12,W,12,'F');
  doc.setTextColor(74,98,128);
  doc.setFontSize(7);
