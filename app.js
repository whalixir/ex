...
// ذخیره نرخ درهم روزانه (هر بار صفحه باز می‌شود ثبت می‌شود)
...
function renderAEDChart(){
  ...
  const target=40000*(1+aedTargetPct/100);
  const targetLine=sorted.map(()=>target);
...
async function generatePDF(){
  ...
  const LOGO_SVG_HTML='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="88" height="88"><defs><linearGradient id="wg1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#1e3a8a"/><stop offset="100%" stop-color="#0f172a"/></linearGradient><linearGradient id="wg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#34d399"/><stop offset="100%" stop-color="#4f8cff"/></linearGradient></defs><circle cx="100" cy="100" r="98" fill="url(#wg1)" stroke="#3b82f6" stroke-width="3"/><circle cx="100" cy="100" r="90" fill="none" stroke="rgba(79,140,255,0.2)" stroke-width="1"/><polyline points="20,140 45,110 68,122 95,82 122,96 150,60 178,42" fill="none" stroke="url(#wg2)" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/><circle cx="20" cy="140" r="5" fill="#34d399"/><circle cx="95" cy="82" r="5" fill="#4f8cff"/><circle cx="178" cy="42" r="5" fill="#34d399"/><text x="100" y="172" text-anchor="middle" font-family="Arial,sans-serif" font-size="16" fill="#93c5fd" font-weight="800" letter-spacing="3">WHALIXIR</text></svg>';
  const logoTag=LOGO_SVG_HTML;
